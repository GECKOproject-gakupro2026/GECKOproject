...
## 付録: 要求4 マイク周波数特性解析（測定完了・2026-07-14）

### 仮説（測定前）
音程ズレ = `CFG_AUDIO_SAMPLE_RATE = 16000U`（`Secure/Core/Src/Inc/app_config.h`）と、PLL3由来のMDF1実クロックの乖離。PLL3設定は `audio_capture.cpp`（PLL3N=80 / PLL3P=28 / PLL3Q=28, DIVQ, source=MSI）。

### 実施した測定
`pc_side/mic_freq_response/` に以下を実装（ファームウェア無変更、PC側のみ）:
- `measure.py`: PCで指定周波数のトーンを`winsound.Beep`で再生しながら、ボードの`'a'`コマンドで音声ストリーミングを開始しCMD_AUDIO(0x03)フレームを受信・連結。numpyのFFT（`np.fft.rfft`、Hanning窓）でピーク周波数を検出し、`f_s_eff = 16000 × (f_meas / f_in)` で実効サンプルレートを逆算する。
- `sweep.py`: 複数周波数で`measure.py`の測定を繰り返し、比（検出周波数/入力周波数）の標準偏差から「純粋なクロックずれ」か「周波数依存の要因（デシメーションフィルタ等）」かを判定する。

**依存はnumpyのみ**（scipyは環境に無かったため、`numpy.fft`で代替。トーン再生はPython標準の`winsound.Beep`を使用、`sounddevice`等の追加インストールは不要にした）。

### 測定結果（300Hz〜3000Hzの5点スイープ、Wi-Fi正常接続環境）

| 入力周波数(Hz) | 検出周波数(Hz) | 比(検出/入力) | 逆算実効レート(Hz) |
|---|---|---|---|
| 300 | 294.0 | 0.9800 | 15680.3 |
| 500 | 490.0 | 0.9800 | 15680.4 |
| 1000 | 980.1 | 0.9801 | 15681.8 |
| 2000 | 1960.5 | 0.9803 | 15684.3 |
| 3000 | 2941.0 | 0.9803 | 15685.2 |

**比の平均 = 0.9802、標準偏差/平均 = 0.01%** — 5点全域でほぼ完全に一定。

### 結論

**周波数依存性はなく、純粋なサンプルレートのズレ**であることが統計的に確定した（デシメーションフィルタ等、周波数帯域によって挙動が変わる要因ではない）。

- 名目サンプルレート: 16000 Hz
- **実測実効サンプルレート: 約15682 Hz**（名目より約1.98%低い）

理論値との突き合わせ（参考、検証はしていない）: PLL3設定（MSI=4MHz想定, M=1, N=80, Q=28）からVCO=320MHz、DIVQ出力=320MHz/28≈11.43MHzとなるが、この値とMDF1のOSR(オーバーサンプリング比)設定から16000Hzちょうどにならない可能性がある。あるいはMSIの実クロック自体が理想値からズレている可能性もある。**根本原因の特定にはMDF1のOSRレジスタ設定値の確認が必要で、これは本測定のスコープ外**。

### 是正案（未実施、別タスク）

1. **名目サンプルレート補正**（最も簡単・ファーム側の修正は不要）: PC側の解析・再生コードで16000Hzの代わりに実測値15682Hzを使う。リアルタイム性を問わない用途（録音・分析）ならこれで十分。
2. **PLL3/MDF1設定の見直し**（ファーム修正が必要）: `audio_capture.cpp`のPLL3パラメータ、またはMDF1のOSR設定を調整し、実効レートが16000Hzちょうどになるよう再計算する。音程を絶対値として使う用途（AI推論の学習データとの整合等）ではこちらが必要になる可能性がある。

### 再現方法
```bash
cd pc_side/mic_freq_response
python measure.py --freq 1000          # 単発測定
python sweep.py --out sweep_result.csv # スイープ測定+CSV出力
```
測定にはボードが正常にWi-Fi接続できている環境が必要（`既知の問題_IDLE復帰の間欠的不安定性.md`の追記参照 — Wi-Fi未接続環境では`pollTcp()`のaccept処理が音声ストリーミングも含めた通信全般を不安定にする）。

### 追記: 基板側リサンプリング補正の実装（2026-07-15、SWD書込み・ファーム変更あり）

上記測定結果を受け、ユーザー指示により**ファームウェア側でPCM出力をリアルタイムに線形補間リサンプリングし、送出前に音程を補正する**実装を行った。

**重要な制約**: この変更は`Secure/Core/`側（`audio_capture.cpp/hpp`, `comm_service.cpp`）のみで完結する。OTA機構はNonSecureアプリのみを更新対象とする設計（Secureは「更新しない基礎領域」）のため、**この補正はWi-Fi経由のOTAでは適用できず、SWD書込みが必須**。

#### 実装

- `audio_capture.hpp/cpp`に`FeedForResample()`/`ResampledAvailable()`/`PopResampled()`のキュー方式APIを追加。512サンプル固定の送信フレームと補正比率のミスマッチを吸収するため、生サンプルを内部キューに投入(Feed)し、送信側は溜まった分を取り出す(Pop)方式にした。
- 位相(`resamplePhase`)をFeed呼び出しをまたいで持ち越す設計にした。単純に「512サンプル→512サンプルを毎回pos=0から補間」する実装では、フレーム境界のたびに末尾の数サンプルが間引かれ、32ms周期の周期的なノイズとなりFFTピークが乱れることを実機で確認した（1000Hz以上で顕著）。前フレーム最後のサンプルを1個保持し「仮想連続ストリーム」として扱うことで解消した。
- `comm_service.cpp`の音声送信部を、DMAイベントごとに`FeedForResample()`→キューに512以上溜まったら`PopResampled()`+送信、という構成に変更した。

#### 補正係数の較正（重要な学び）

**「FFT音程ズレから逆算した実効サンプルレート(15682.4Hz)をそのまま補正比率として使う」という素朴なアプローチは誤りだった。** 実際にファームでその比率(0.9802)を使って線形補間したところ、FFTスイープの結果はさらに悪化した(比0.9613)。原因を`printf`デバッグで追跡した結果、**DMAのhalf/full-complete割り込み頻度が、音程ズレから予想される頻度より約4.5%速い**ことが判明した(理論15.31Hz vs 実測16.0Hz)。つまり「サンプルの中身の周波数成分のズレ」と「DMAバッファが埋まる速さ」は同じ根本原因(MSIクロック)から来ているはずだが、単純な比例関係では説明がつかない差があり、根本原因は特定できていない。

このため、**補正係数(`kResampleStep`)は理論値からの逆算ではなく、`sweep.py`の出力を見ながら実機で直接チューニングする方式**に切り替えた。最終的に`kResampleStep = 1.0196`（1000Hzで比率が0.9613→1.0000になるまで調整）で確定した。

#### 最終検証結果

300Hz〜4000Hzの8点スイープで**比0.9996〜1.0000**（誤差0.04%以内）を達成。200Hzのみ外れ値(0.26)だが、これはFFT解析側の限界（低周波数での周波数分解能・環境ノイズの影響）であり補正ロジックの問題ではないと判断。

```
freq_in  peak_meas  ratio   eff_rate
  300.0      299.9  0.9996  15993.3
  500.0      500.0  1.0000  16000.0
  700.0      699.7  0.9996  15994.1
 1000.0      999.7  0.9997  15994.7
 1500.0     1499.7  0.9998  15996.6
 2000.0     1999.7  0.9998  15997.5
 3000.0     2999.4  0.9998  15996.6
 4000.0     3999.4  0.9998  15997.4
```
(全データは`pc_side/mic_freq_response/sweep_final.csv`)

実機検証: `verify_regression.py`全項目PASS（Secure変更のみなのでNonSecureは無変更、SWD再書込みで確認）。

#### 今後の注意

- `kResampleStep`はこのボード個体で較正した値。**マイクや基板を変える場合は`sweep.py`で再較正すること**（`audio_capture.cpp`のコメント参照）。
- DMA割り込み頻度と音程ズレの不一致の根本原因は未解明。将来的にMDF1/PLL3クロック系統をより深く調査する余地がある。
