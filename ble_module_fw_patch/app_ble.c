...
/* USER CODE END APP_BLE_Init_1 */
  ...
  /* Bug fix (v1.18 BLE_AT_Server): Adv_Mgr's 60 s timer raises
   * CFG_TASK_ADV_UPDATE_ID but the task was never registered, so the
   * sequencer branched to a NULL pointer -> HardFault, killing the AT UART
   * while CPU2 kept advertising. Register a handler that simply keeps the
   * current advertising mode running. */
...
SVCCTL_UserEvtFlowStatus_t SVCCTL_App_Notification( void *pckt ) 
{
  ...
  switch (event_pckt->evt)
  {
    case HCI_DISCONNECTION_COMPLETE_EVT_CODE:
    {
      ...
      /* Restart advertising so the device stays discoverable after the
       * central drops (the stock AT server left the device idle forever) */
...
/* Runs in the sequencer when the advertising timer elapses; the stock v1.18
 * sources forgot to register any handler for it (NULL call -> HardFault).
 * Keep advertising unchanged so the board stays discoverable. */
static void Adv_Update( void )
{
  return;
}
...
static void Manage_Update_Charac(void)
{
  ...
  if (global_notif_serv_index == 1)
  {
    ...
    {
      /* Was: only a 2-byte button notification when val_tab[0]==1, which
       * silently discarded any real payload. Forward the payload as-is so
       * the host can push telemetry frames to the central. */
      extern uint8_t global_notif_val_tab_len;
      result = P2PS_STM_Notify_Raw(&global_notif_val_tab[0], global_notif_val_tab_len);
      if (result != BLE_STATUS_SUCCESS)
      {
        printf("P2PS_STM_Notify_Raw failed: 0x%02x \r\n\r", result);
      }
    }
  }
