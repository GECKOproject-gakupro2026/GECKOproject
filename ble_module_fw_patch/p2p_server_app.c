...
#include <string.h> /* memcpy for the raw BLE_EVT_WRITE forwarding below */
...
/* USER CODE BEGIN P2PS_STM_WRITE_EVT */
      ...
      /* Forward the raw write payload to the U585 host as-is (frame_codec
       * REC_START/STOP frames use SOF=0xAA, which never matches the
       * end-device-select bytes (0x01..0x06) the stock demo branches below
       * look for) so stm32wb_at_BLE_EVT_WRITE_cb() on the U585 side actually
       * sees every GATT write, not just the 2-byte LED demo command. */
      if (pNotification->DataTransfered.Length > 0U &&
          pNotification->DataTransfered.Length <= sizeof(param_BLE_EVT_WRITE.val_tab))
      {
        param_BLE_EVT_WRITE.svc_index = 1;
        param_BLE_EVT_WRITE.char_index = 1;
        param_BLE_EVT_WRITE.val_tab_len = pNotification->DataTransfered.Length;
        memcpy(param_BLE_EVT_WRITE.val_tab, pNotification->DataTransfered.pPayload,
